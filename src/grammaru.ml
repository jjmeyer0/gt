...
module Grammaru = struct 
  ...
  let rec get_nonterminal_strings (Grammaru(_,_,productions,_,_):grammaru) : 'a list =
  ...
  let rec gu2ge (Grammaru(n,lcd,pl,lcl,hs) as g:grammaru) : Grammare.grammare = 
	 let r = ref Tnone in
	 ...
	 Grammare.Grammare(n,ss,lcd,prods,lcs',!r,hs)
  ;;
end
